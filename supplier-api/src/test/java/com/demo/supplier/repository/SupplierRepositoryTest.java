...
class SupplierRepositoryTest {
  ...
  void save_persistsProduct() {
    SupplierProduct product = buildProduct("Wireless Headphones", "99.90");
  ...
  void findById_returnsProduct() {
    SupplierProduct product = em.persistAndFlush(buildProduct("USB-C Hub", "49.90"));

    Optional<SupplierProduct> found = repository.findById(product.getId());
  ...
  void findAll_returnsAllSavedProducts() {
    ...
    List<SupplierProduct> result = repository.findAll();
  ...
  void save_updatesExistingProduct() {
    SupplierProduct product = em.persistAndFlush(buildProduct("Old Name", "10.00"));
    ...
    SupplierProduct updated = repository.findById(product.getId()).orElseThrow();
  ...
  void delete_removesProduct() {
    SupplierProduct product = em.persistAndFlush(buildProduct("To Delete", "5.00"));
  ...
  private SupplierProduct buildProduct(String name, String price) {
